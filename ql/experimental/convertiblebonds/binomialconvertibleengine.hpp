...
namespace QuantLib {
    ...
    */
	...
	//!Binomial Tsiveriotis - Fernandes engine for coco bonds
		//A separate class is needed because a different discretized asset is needed (DiscretizedCoCo)
		/*  \ingroup hybridengines
		...
		*/
		template <class T>
	class BinomialCoCoEngine : public ContingentConvertible::optionCoCo::engine {
	public:
		BinomialCoCoEngine(
			const ext::shared_ptr<GeneralizedBlackScholesProcess>& process,
			Size timeSteps, bool isCocoWriteDown)
			: process_(process), timeSteps_(timeSteps) {
			QL_REQUIRE(timeSteps>0,
				"timeSteps must be positive, " << timeSteps <<
				" not allowed");
			registerWith(process_);
			isCocoWriteDown_ = isCocoWriteDown;
		}
		void calculate() const;
	private:
		ext::shared_ptr<GeneralizedBlackScholesProcess> process_;
	...
	template <class T>
	void BinomialCoCoEngine<T>::calculate() const {
		...
		ext::shared_ptr<Lattice> lattice(
			new TsiveriotisFernandesLattice<T>(tree, riskFreeRate, maturity,
				timeSteps_, creditSpread, v, q, isCocoWriteDown_)
		);
		DiscretizedCoCo coco(arguments_, bs,
			TimeGrid(maturity, timeSteps_));

		coco.initialize(lattice, maturity);
		coco.rollback(0.0);
		results_.value = coco.presentValue();
		QL_ENSURE(results_.value < std::numeric_limits<Real>::max(),
			"floating-point overflow on tree grid");
	}
